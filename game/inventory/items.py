...
class ItemDefinition:
    ...
    defence_bonus: int = 0
    # Firearms only: the ammunition they are chambered for. A gun with no
    # rounds of this key in the player's inventory is dead weight, and
    # contributes nothing to a fight.
...
ITEMS = (
    ...
    ItemDefinition("brick", "Half Brick", "weapon", "Loose masonry, thrown once and gone.", "items/brick.png", True, 20, 40, equipment_slot="throwable", strength_bonus=14),
    ItemDefinition("glass_bottle", "Glass Bottle", "weapon", "An empty bottle that becomes a weapon on impact.", "items/glass-bottle.png", True, 20, 65, equipment_slot="throwable", strength_bonus=18),
    ItemDefinition("flash_banger", "Flash Banger", "weapon", "Surplus stun charge. Loud, blinding, single use.", "items/flash-banger.png", True, 10, 340, equipment_slot="throwable", strength_bonus=32),
    ItemDefinition("petrol_bomb", "Petrol Bomb", "weapon", "A rag, a bottle and a very short future.", "items/petrol-bomb.png", True, 10, 520, equipment_slot="throwable", strength_bonus=44),
    ItemDefinition("police_baton", "Police Baton", "weapon", "A balanced close-range weapon.", "items/police-baton.webp", False, 1, 620, equipment_slot="melee", strength_bonus=8),
    ItemDefinition("tire_iron", "Tyre Iron", "weapon", "Compact steel with a punishing swing.", "items/tyre-iron.webp", False, 1, 760, equipment_slot="melee", strength_bonus=10),
    ItemDefinition("hatchet", "Hatchet", "weapon", "A short, heavy chopping weapon.", "items/hatchet.webp", False, 1, 980, equipment_slot="melee", strength_bonus=12),
    ItemDefinition("survival_knife", "Survival Knife", "weapon", "A stronger secondary blade.", "items/survival-knife.webp", False, 1, 880, equipment_slot="melee", strength_bonus=8),
    # Ammunition. Sold by the round, spent one per fight.
    ItemDefinition("ammo_22", ".22 Rounds", "utility", "Rimfire rounds for a pocket pistol.", "items/ammo-22.webp", True, 200, 10),
    ItemDefinition("ammo_9mm", "9mm Rounds", "utility", "The calibre every other pistol in London is chambered for.", "items/ammo-9mm.webp", True, 200, 16),
    ItemDefinition("ammo_38", ".38 Rounds", "utility", "Revolver rounds, harder to come by than nine-mil.", "items/ammo-38.webp", True, 200, 20),
    # Guns v1.0: low-calibre pistols, the first firearms on the ladder.
    ItemDefinition("derringer_22", "Derringer .22", "weapon", "A palm-sized two-shot pistol with no range to speak of.", "items/derringer-22.webp", False, 1, 2000, equipment_slot="secondary", strength_bonus=16, ammo_key="ammo_22"),
    ItemDefinition("converted_blank_pistol", "Converted Blank Pistol", "weapon", "A starter pistol bored out in somebody's shed.", "items/converted-blank-pistol.webp", False, 1, 2800, equipment_slot="secondary", strength_bonus=18, ammo_key="ammo_9mm"),
    ItemDefinition("snub_nose_38", "Snub-Nose .38", "weapon", "A short-barrelled revolver that disappears under a coat.", "items/snub-nose-38.webp", False, 1, 3600, equipment_slot="primary", strength_bonus=20, ammo_key="ammo_38"),
    ItemDefinition("compact_9mm", "Compact 9mm", "weapon", "A clean semi-automatic that has never been fired in anger.", "items/compact-9mm.webp", False, 1, 4800, equipment_slot="primary", strength_bonus=22, ammo_key="ammo_9mm"),
...
# An item is ammunition because a weapon is chambered for it, so the
# weapons stay the single source of truth for what counts as a round.
AMMO_KEYS = frozenset(
    item.ammo_key for item in ITEMS if item.ammo_key is not None
)
...
def is_ammo(item_key):
    return item_key in AMMO_KEYS


def weapons_using(ammo_key):
    return tuple(item for item in ITEMS if item.ammo_key == ammo_key)
